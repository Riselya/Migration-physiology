...
##good threads on data transformation
#https://github.com/joey711/phyloseq/issues/492
#https://github.com/joey711/phyloseq/issues/283

#To download phyloseq and DESeq2 from bioconductor run following two lines:

#source('http://bioconductor.org/biocLite.R')
#biocLite('phyloseq')
#biocLite("DESeq2")

#It might mess up some other packages, so packages may need reinstalling (either from CRAN or Bioconductor) as errors come up when loading phyloseq and DESeq2 into library
#I had to reinstall ~10 packages

##setwd() somewhere and download all github files to this location
...
##subset samples to just birds (there are environmental samples in here too)
...
## data has more samples than we need. We are going to prune the dataset to just the relevant samples for this analysis

## for migration analysis (migrant/resident comparison)
...
## 77 samples, 5262 OTUs


############# DESEq 2 ###################################################################

## Will do this analysis first (for Fig2 and Table S3) before rarefying data

## Analyse differential abundances between migrants and residents for each group

## subset RNS (Red-necked stint) in Broome

rns_br<-migration%>%subset_samples(species=="Red-necked stint")
rns_br<-rns_br%>%subset_samples(site=="Broome")
rns_br <- prune_taxa(taxa_sums(rns_br) > 0, rns_br)

#########################

##the following analysis is from the Phyloseq-DESeq2 vignette, linked at top of script.

diagdds = phyloseq_to_deseq2(rns_br, ~ age)
...
sigtab_rns.br = cbind(as(sigtab, "data.frame"), as(tax_table(rns_br)[rownames(sigtab), ], "matrix"))
head(sigtab_rns.br)
sigtab_rns.br<- sigtab_rns.br[order(sigtab_rns.br$log2FoldChange),]

#add variable to end
sigtab_rns.br$Group<-"Red-necked stint Broome"
#########
...
rns_vic<-migration%>%subset_samples(species=="Red-necked stint")
rns_vic<-rns_vic%>%subset_samples(month=="September")
rns_vic <- prune_taxa(taxa_sums(rns_vic) > 0, rns_vic)

########################

diagdds = phyloseq_to_deseq2(rns_vic, ~ age)
...
alpha = 0.01
sigtab = res[which(res$padj < alpha), ]
sigtab_rns.vic = cbind(as(sigtab, "data.frame"), as(tax_table(rns_vic)[rownames(sigtab), ], "matrix"))
head(sigtab_rns.vic)
sigtab_rns.vic<- sigtab_rns.vic[order(sigtab_rns.vic$log2FoldChange),]

sigtab_rns.vic$Group<-"Red-necked stint Flinders"

####################################################

#curlew sandpiper

cs<-migration%>%subset_samples(species=="Curlew sandpiper")
cs <- prune_taxa(taxa_sums(cs) > 0, cs)

#####################

diagdds = phyloseq_to_deseq2(cs, ~ age)
...
alpha = 0.01
sigtab = res[which(res$padj < alpha), ]
sigtab_cs = cbind(as(sigtab, "data.frame"), as(tax_table(cs)[rownames(sigtab), ], "matrix"))
head(sigtab_cs)
sigtab_cs<- sigtab_cs[order(sigtab_cs$log2FoldChange),]

sigtab_cs$Group<-"Curlew sandpiper Broome"
...
deseq2<-rbind(sigtab_rns.br, sigtab_rns.vic, sigtab_cs)

#edit small details for figure

deseq2[3,11]<-"Corynebacteriaceae"
deseq2[12,11]<-"Corynebacteriaceae"
revalue(deseq2$Family, c("Gammaproteobacteria_unclassified"="Gammaproteobacteria uncl."))

   
deseq2$Family<-factor(deseq2$Family, levels = c("Corynebacteriaceae",
                                                 "Clostridiaceae_1","Lachnospiraceae" ,"Peptostreptococcaceae",  "Ruminococcaceae",   
          
                                                  "Moraxellaceae",              
                                                  "Campylobacteraceae"  ,       
                                                  "Helicobacteraceae" ,         
                                                  "Rhodocyclaceae"   ,          
                                                  "Gammaproteobacteria uncl.",
                                                  "Succinivibrionaceae",
                                                  "Chitinophagaceae" ,
                                                  "Mycoplasmataceae")) 

deseq2$Phylum<-factor(deseq2$Phylum, levels = c("Actinobacteria","Firmicutes", "Proteobacteria","Bacteroidetes","Tenericutes"))



#Figure 2

ggplot(deseq2, aes(x=Family, y=log2FoldChange, color=Phylum)) + 
  geom_jitter(aes(size = baseMean), width = 0.2, alpha = 0.5)+ scale_size(range = c(2,10))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  geom_hline(yintercept = 0, linetype = 2)+facet_grid(.~Group, scales = "free")+ylim(-26,26)+
  scale_color_manual(values = c("navyblue","green4","orchid","royalblue","burlywood"))+
  theme( axis.text = element_text(size=12), axis.title=element_text(size=14))+
  theme(plot.margin=unit(c(0.5,1,1,2),"cm"))+guides(color = guide_legend(override.aes = list(size=5)))+
...
##Now rest of analyses

migration1 <-migration

##rarefy
migration <-rarefy_even_depth(migration1, sample.size = min(sample_sums(migration1)),
                          rngseed = 3, replace = TRUE, trimOTUs = TRUE, verbose = TRUE)

migration

migration2 <- migration1 %>%
  scale_reads(n=5815) ##scale to smallest library size

#migration1 = unrarefied
#migration = rarefied
#migration2 = scaled

#using rarefied data, but can test with others - makes no difference
...
prevdf = data.frame(Prevalence = prev0,
                    ...
                    tax_table(migration))
keepPhyla = table(prevdf$Phylum)[(table(prevdf$Phylum) > 10)]
prevdf1 = subset(prevdf, Phylum %in% names(keepPhyla))

prevdf1$Prevalence<-(prevdf1$Prevalence/77)*100
prevdf$Prevalence<-(prevdf$Prevalence/77)*100

ggplot(prevdf1, aes(TotalAbundance, Prevalence)) +
  geom_point(size = 2, alpha = 0.2) +
  scale_x_log10() +
  xlab("Total Abundance") + ylab("Prevalence across all birds (%)")+
  facet_wrap(~Phylum)+theme_bw() +
  theme(  axis.text.y = element_text(size=12), axis.title=element_text(size=14))+ylim(c(0,100))

mean(prevdf$Prevalence)
...
#Subset each group seperately to get top 10 OTUs
# b = broome, f = flinders, a = adult/migrants, sy = second year/resident

curlew.b<-migration%>%subset_samples(species=="Curlew sandpiper"& site=="Broome")
...
stint.b<-migration%>%subset_samples(site=="Broome" & species=="Red-necked stint")
...
stint.f<-migration%>%subset_samples(month=="September")
...
prevdf$RelAbund<-prevdf$TotalAbundance/sum(prevdf$TotalAbundance)
prevdf$RelPrev<-prevdf$Prevalence/12
prevdf<-prevdf[order(-prevdf$RelAbund),]
head(prevdf)
...
prevdf$RelAbund<-prevdf$TotalAbundance/sum(prevdf$TotalAbundance)
prevdf$RelPrev<-prevdf$Prevalence/6
prevdf<-prevdf[order(-prevdf$RelAbund),]
head(prevdf)
...
prevdf$RelAbund<-prevdf$TotalAbundance/sum(prevdf$TotalAbundance)
prevdf$RelPrev<-prevdf$Prevalence/13
prevdf<-prevdf[order(-prevdf$RelAbund),]
head(prevdf)
...
prevdf$RelAbund<-prevdf$TotalAbundance/sum(prevdf$TotalAbundance)
prevdf$RelPrev<-prevdf$Prevalence/16
prevdf<-prevdf[order(-prevdf$RelAbund),]
head(prevdf)
...
prevdf$RelAbund<-prevdf$TotalAbundance/sum(prevdf$TotalAbundance)
prevdf$RelPrev<-prevdf$Prevalence/15
prevdf<-prevdf[order(-prevdf$RelAbund),]
...
ggplot(alphadiv_rich_observed, aes(x = age, y = mean))+geom_boxplot(width=0.5)+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+
  facet_wrap(species~site, scales="free", nrow=1, ncol=3)
